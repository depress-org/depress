...
export async function runMigrate(options: MigrateOptions) {
  ...
  // Step 3b: Generate redirect map
  const redirectMap: Record<string, string> = {}
  const netlifyRedirects: string[] = ['# Generated by depress — do not edit manually']
  for (const post of wpExport.posts) {
    ...
    for (const old of oldPaths) {
      if (old !== newPath) {
        redirectMap[old] = newPath
        netlifyRedirects.push(`${old}  ${newPath}  301`)
      }
    }
  }
  await mkdir(outputDir, { recursive: true })
  await writeFile(join(outputDir, 'redirects.json'), JSON.stringify(redirectMap, null, 2) + '\n', 'utf-8')
  await writeFile(join(outputDir, '_redirects'), netlifyRedirects.join('\n') + '\n', 'utf-8')
  ...
  if (adapter) {
    ...
    console.log(`  Redirects: ${chalk.bold(String(Object.keys(redirectMap).length))} entries → redirects.json + _redirects`)
    console.log()
